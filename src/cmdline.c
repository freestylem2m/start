		...
		case A_FLAG:
			if (args[ix]._vec.f_ptr.f_mask == 0) {
				if (*args[ix]._vec.f_ptr.f_ptr > 1) {
					printf(" --%s [repeated %d]\n", args[ix]._str[1] == '|' ? &args[ix]._str[2] : args[ix]._str,
						   *args[ix]._vec.f_ptr.f_ptr);
				} else {
					printf(" --%s [%s]\n", args[ix]._str[1] == '|' ? &args[ix]._str[2] : args[ix]._str,
						   *args[ix]._vec.f_ptr.f_ptr ? "enabled" : "disabled");
				}
			} else {
				printf(" --%s [%s]\n", args[ix]._str[1] == '|' ? &args[ix]._str[2] : args[ix]._str,
					   *args[ix]._vec.f_ptr.f_ptr & args[ix]._vec.f_ptr.f_mask ? "enabled" : "disabled");
			}
			break;
		default:
			FATAL("WTF?");
...
int parse_cmdline(int ac, char *av[])
{
	...
	int                 ix, sx;	// ix indexes args[] table, sx indexes binary_scale[] table.
				...
				case A_LONG:
					*(args[ix]._vec.l_ptr) = strtol(_v, &_p, 0);
					...
					for (sx = 0; _p && *_p && binary_scale[sx].c; sx++) {
						if (*_p == binary_scale[sx].c) {
							*(args[ix]._vec.l_ptr) *= binary_scale[sx].scale;
							_p++;
						}
					}
					break;
